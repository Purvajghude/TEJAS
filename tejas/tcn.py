...
# ----------------------------------------------------------------------
# Causal residual TCN (the ensemble's deep member).  Unlike TCNNet above,
# this is *strictly causal*: each conv left-pads by (k-1)*dilation and chomps
# the right overhang, so position t never sees t+1.  Residual blocks with
# dilations 1,2,4,8,16,32 give a receptive field that spans the whole window.
# ----------------------------------------------------------------------
class Chomp1d(nn.Module):
    """Drop the right overhang created by causal left-padding."""
...
class CausalBlock(nn.Module):
    def __init__(self, in_ch, out_ch, k, dilation, p):
        super().__init__()
        pad = (k - 1) * dilation
        self.net = nn.Sequential(
            nn.Conv1d(in_ch, out_ch, k, padding=pad, dilation=dilation), Chomp1d(pad),
            nn.BatchNorm1d(out_ch), nn.ReLU(), nn.Dropout(p),
            nn.Conv1d(out_ch, out_ch, k, padding=pad, dilation=dilation), Chomp1d(pad),
            nn.BatchNorm1d(out_ch), nn.ReLU(), nn.Dropout(p),
        )
...
class CausalTCN(nn.Module):
    def __init__(self, in_ch=3, hidden=32, dilations=(1, 2, 4, 8, 16, 32), k=3, p=0.2):
        super().__init__()
        layers, ch = [], in_ch
        for d in dilations:
            layers.append(CausalBlock(ch, hidden, k, d, p))
            ch = hidden
        self.body = nn.Sequential(*layers)
        self.head = nn.Linear(hidden, 1)

    def forward(self, x):
        h = self.body(x)        # (B, hidden, W)
        h = h[:, :, -1]         # causal readout: last step has seen the full window
